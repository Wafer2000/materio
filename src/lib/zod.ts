...
export const RegisterSchema = object({
  ...
  avatar: string({ required_error: "Se requiere una URL de avatar" }).url(
    "La URL de avatar no es válida"
  ),
  birthdate: date({ required_error: "Se requiere una fecha de nacimiento" }),
  id_card: string({ required_error: "Se requiere un número de cédula" })
    .transform((v) => Number(v) || 0),
  phone: string({
    required_error: "Se requiere un número de teléfono",
  })
    .min(1, "Se requiere un número de teléfono")
    .max(20, "El número de teléfono debe tener menos de 20 caracteres")
    .refine((value) => RPNInput.isValidPhoneNumber(value), {
